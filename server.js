...
if (!adminExists) {
  ...
  ).run('Letícia Rufino', 'admin@leticia.com', hash);
...
/* GET /api/posts?category=&status=published&page=1&limit=9 */
app.get('/api/posts', (req, res) => {
  ...
  const rows = db.prepare(`
    SELECT p.id, p.title, p.slug, p.excerpt, p.cover_url,
           p.featured, p.read_time, p.status,
...
/* ══════════════════════════════════════════
   POSTS — rotas protegidas (admin)
══════════════════════════════════════════ */

/* GET /api/admin/posts  (todos os status) */
app.get('/api/admin/posts', authMiddleware, (req, res) => {
  ...
  const total = db.prepare(`SELECT COUNT(*) as n FROM posts p ${whereStr}`).get(...params).n;
  const rows  = db.prepare(`
    SELECT p.id, p.title, p.slug, p.status, p.featured, p.read_time,
           p.created_at, p.updated_at,
...
/* POST /api/admin/upload  — upload de imagem de capa */
